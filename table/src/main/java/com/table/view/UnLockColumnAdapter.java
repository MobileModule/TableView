...
public class UnLockColumnAdapter extends RecyclerView.Adapter<UnLockColumnAdapter.UnLockViewHolder> {
    ...
    @Override
    public void onBindViewHolder(UnLockViewHolder holder, @SuppressLint("RecyclerView") final int position) {
    ...
    private void createRowView(LinearLayout linearLayout, List<String> datas, boolean isFristRow, int mMaxHeight) {
        ...
        for (int i = 0; i < datas.size(); i++) {
            ...
            if (i != datas.size() - 1) {
                ...
                ViewGroup.LayoutParams splitViewParmas = new ViewGroup.LayoutParams(DisplayUtil.dip2px(mContext, 0.5f),
                        ViewGroup.LayoutParams.MATCH_PARENT);
                splitView.setLayoutParams(splitViewParmas);
                if (isFristRow) {
                    splitView.setBackgroundColor(ContextCompat.getColor(mContext, R.color.transparent));
                } else {
                    splitView.setBackgroundColor(ContextCompat.getColor(mContext, R.color.transparent));
                }
